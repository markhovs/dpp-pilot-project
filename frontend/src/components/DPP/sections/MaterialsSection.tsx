import {
  ...
  Badge, Accordion, AccordionItem, AccordionButton, AccordionPanel, AccordionIcon,
  Table, Tbody, Tr, Td, Th, TableContainer, List, ListItem, ListIcon, Divider
} from "@chakra-ui/react";
import { MdDeveloperMode, MdCheck, MdClose, MdCircle, MdLayers } from "react-icons/md";
...
const MaterialsSection: React.FC<MaterialsSectionProps> = ({
  ...
  const renderComponentStructure = (component: any, level = 0) => {
    ...
    if (BulkCount !== undefined) {
      ...
      const bulkCountValue = BulkCount?.value !== undefined && BulkCount.value !== null
        ? BulkCount.value
        : "-";
          ...
          <AccordionItem border="none">
            ...
            <AccordionPanel pt={4}>
              ...
              {material && Object.keys(material).length > 0 && (
                <Box mb={4}>
                  <Heading size="sm" mb={2}>Material Composition</Heading>
                  <TableContainer>
                    <Table variant="simple" size="sm">
                      <Tbody>
                        {Object.entries(material).map(([key, value]) => (
                          <Tr key={key}>
                            <Th width="30%">{key}</Th>
                            <Td>{typeof value === 'object' ? getFirstLangValue(value) : String(value)}</Td>
                          </Tr>
                        ))}
                      </Tbody>
                    </Table>
                      ...
                      <Tbody>
                        {Object.entries(displayProps).map(([key, value]) => (
                          <Tr key={key}>
                            <Th width="30%">{key}</Th>
                            <Td>
                              {value === null ? "None" : getPropertyDisplayValue(value)}
                            </Td>
                          </Tr>
                        ))}
                      </Tbody>
              ...
              {developerMode && (
                <Box mb={4}>
                  <Heading size="sm" mb={2}>Technical Details</Heading>
              ...
              {components && components.length > 0 && (
                <Box>
                  <Heading size="sm" mb={2}>Sub-Components</Heading>
                  {components.map((subComponent: any) =>
  ...
  return (
    <VStack spacing={6} align="stretch">
      ...
      {archeType && (
        <Box p={5} shadow="md" borderWidth="1px" borderColor={borderColor} borderRadius="lg" bg={cardBg}>
          <Heading size="md" mb={4}>Archetype</Heading>
          <Text>{archeType}</Text>
        </Box>
      )}
      ...
      {recycling && Object.keys(recycling).length > 0 && (
        <Box p={5} shadow="md" borderWidth="1px" borderColor={borderColor} borderRadius="lg" bg={cardBg}>
          ...
          {recycling.materials && recycling.materials.length > 0 && (
            <Box>
              <Heading size="sm" mb={2}>Recyclable Materials</Heading>
              <List spacing={2}>
